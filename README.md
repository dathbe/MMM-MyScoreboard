...
| `channelTiers`         | An ordered list of channel preference tiers, from highest to lowest priority. After the existing market/channel rules and `skipChannels` are applied, only channels from the first tier containing an available channel are displayed. If multiple available channels are in the winning tier, all of them remain eligible. Tiers are generic and may contain any normalized channel names; they do not have built-in meanings such as local, national, or streaming. An empty array disables tier filtering and preserves current behavior.<br><br>Example: `[['YES', 'MSG'], ['ESPN', 'TNT'], ['ESPN Unlmtd']]`.<br><br>**Type** `Array` of `Arrays` of `Strings`<br>**Default** `[]`
| `showUnmatchedChannels` | When `channelTiers` is configured but none of its tiers contain an available channel, show the channels that survived normal filtering instead of hiding them. This makes newly-added or unexpected services visible so they can be added to `channelTiers`. Has no effect when `channelTiers` is empty.<br><br>**Type** `Boolean`<br>**Default** `false`
| `maxChannels`          | Maximum number of eligible broadcast channels to cycle through for each game. This setting is independent of `channelTiers` and applies to broadcasts from any provider. `0` means unlimited; `1` means at most one channel; `2` means at most two channels; etc.<br><br>**Type** `Number`<br>**Default** `0`
| `displayLocalChannels` | A list of local channels you would like to display, even if `showLocalBroadcasts` is set to `false`.  This allows you to manually set the local broadcast channels you receive.  I.e., start with no channels and add the ones you want.  `displayLocalChannels` will override other options to turn off channels, except for the absolute `skipChannels` veto.<br><br>**Type** `Array` or `Strings`<br>**Default** `[]`
...
- `node --run test:e2e` - Run the curated Playwright matrix (26 scenarios, ~1–2 min). Screenshots land in `test-results/screenshots/`.
